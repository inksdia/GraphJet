...
public class ESRepo {
    ...
    public SearchRequestBuilder getBuilder(String query) {
        ...
        if (query != null) {
            ...
            searchRequestBuilder.setQuery(QueryBuilders.termQuery("snT", "TWITTER")).setPostFilter(FilterBuilders.queryFilter(queryBuilder));
        }
    ...
    public Message parseHit(SearchHit hit) {
        try {
            ...
            if (sourceMap.get("mTp").equals(8)) {
                Object obj = ((Map) sourceMap.get("add")).get("qsId");
                if (obj != null) {
                    message.setRetweet(true);
                    message.setRetweetId(Long.valueOf((String) obj));
                }
            } else {
